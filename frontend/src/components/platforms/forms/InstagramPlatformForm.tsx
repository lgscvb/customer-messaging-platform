...
import { InstagramPlatformConfig, PlatformStatus } from '../../../types/platform';
import platformService from '../../../services/platformService';

/**
 * Instagram 平台表單屬性接口
 */
interface InstagramPlatformFormProps {
  initialData?: Partial<InstagramPlatformConfig>;
 ...
 * Instagram 平台表單組件
 */
const InstagramPlatformForm: React.FC<InstagramPlatformFormProps> = ({
  initialData,
  onSubmit,
  onCancel,
  isEdit = false
  ...
  // 初始化表單數據
  const defaultValues = platformService.getInstagramFormInitialValues();
   ...
   */
  const getWebhookUrl = () => {
    return `${process.env.NEXT_PUBLIC_API_URL || 'https://api.example.com'}/webhook/instagram`;
  };
  ...
  return (
    <Box component="form" onSubmit={handleSubmit}>
      ...
      <Paper elevation={0} sx={{ p: 3, mb: 3, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
        <Typography variant="h6" fontWeight="bold" gutterBottom>
          {t('platforms.instagramCredentials')}
        </Typography>
        
        <Alert severity="info" sx={{ mb: 3 }}>
          <Typography variant="body2">
            {t('platforms.instagramCredentialsInfo')}
            <Link 
      ...
      <Paper elevation={0} sx={{ p: 3, mb: 3, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
        ...
        <Alert severity="warning" sx={{ mb: 3 }}>
          {t('platforms.instagramWebhookInfo')}
        </Alert>
...
export default InstagramPlatformForm;